...
- [2026-08-22] `GKD-M3-C` fixed head 已独立验收、合并并刷新 seven-Skill project staging。
  - Why: trusted main 对 PR #18 的完整 diff、requirements、delivery、candidate bundle/evidence、review core 五轴代码审查、独立 389 项 verifier、隔离 bundle install 和 live fixed-head `GKD Verify` 进行了终验，无阻塞 finding。
  - Impact: fixed head `05eae98d2c5d1c0e16e16a09c958cb9d2c6480b4` 以 squash merge `6f265e1626ddb4255051a35cd6065afdf9ef48a5` 进入 main；accepted bundle/evidence digest 为 `b93568270185a44d5a39855a50e354eb22624ad9d7e4a896e87b0aff99d98487` / `816c9cb9bac95f31472fb84e821d13229ff54346830255a5a19c4430c069c2f9`。M3 完成，M4 只能从当前 fresh main 与更新后的 role/config/staging digest 通过完整 automatic bridge 启动。
...
- [2026-08-24] R9 通过独立 R10 release task 发布为稳定 `v0.1.5`。
  - Why: R9 修复 GitHub merged/closed 解析、canonical stdout 尾随换行、exact-head squash reconciliation 和 delivery document 顺序，但 source `0.1.4` 不能被当作已发布 bundle。R10 从 verified `v0.1.4` asset 启动，独立完成 version-only candidate、434 项 verifier、fixed-head CI、canonical acceptance 与 post-merge L3/L4。
  - Impact: PR #33 fixed head `81ee21751634d5c4609db6313a73353e8221e65d` 经 canonical `gkd-task accept --merge` merge 为 `60ac0c49f1054ce2edea49b3ab6758bfbd3432b3`。`v0.1.5` tag、Release 和 asset `gkd-0.1.5-final-60ac0c4-a.tar.gz` 均精确绑定该 source SHA；bundle/asset SHA-256 为 `d749b753fb11aeab44d41b4e1d8bec44c7fa2d18a4b08148fbc0e0c127e27e6d` / `f259475f4ca6c3425e53d734d03633541d6a1997e41991eb5a6115958d06a298`。asset-local install/verify 后，GKD project staging inventory 为 `c8e2e37b8c21655202adf9595120b31c098ae1846a39c261d3c0ac8fe2c8180e`；R10 candidate、任务分支、sandbox canary PR/branch 与临时根已清理，生产 `~/.codex` 和 AIO 未写入。AIO 现在只能先升级到该已发布 pin，再继续其剩余计划。

- [2026-08-24] AIO B4 adapter policy 已完成独立 rework、fixed-head acceptance、merge 与 records-only closeout。
  - Why: B4 首轮验收因绝对 policy path 触发 `POLICY_PATH_UNSUPPORTED`；manual recovery 后一次独立 monitor 发生 `GITHUB_QUERY_FAILED`，下一次 executor 又以简写 repository 触发 `REPOSITORY_INVALID`。这些失败均是固定流程事实，不能改写为代码成功或沿同一 attempt 重试。
  - Impact: canonical rework 退役旧 delivered attempts 并以 epoch 3 新 offer/claim 重新交付；PR #177 fixed head `58617c03142c63cd3f4017ef1fea6b2cb0e5f97f` 通过正确完整 repository、相对 `.gkd/policy.json`、3600 秒 monitor 和独立 acceptance，squash merge 为 `36a4b1855e767ce89286950b355cf249918eb732`。records-only PR #178 fixed head `cd966c1236bf64641a7bd6969823b4a4ed583740` 通过同一 3600 秒 closeout monitor，merge 为 `3f856c88749f4875889164fa72caeebc22143d98`。B4 未修改 AIO 原始未跟踪计划目录、GKD canonical source、生产目录或 GitHub settings；下一步是 adoption C。

- [2026-08-25] AIO C history adapter 已完成两轮 canonical rework、独立 acceptance、merge 与 records-only closeout。
  - Why: 首轮候选只在 manifest-only task.json 变化时触发 history smoke，随后又被发现删除路径被 `ACMR` diff filter 排除；两项均由新的 canonical attempt/revision 修复，不能把旧 fixed head 当作最终交付。
  - Impact: PR #179 fixed head `0d91548c1990dd059ba6df0f6465e2c275714f36` 以 `378fb51569a904573c8a95a9e0b5b51e6820de88` 合并；独立 monitor 使用相对 `.gkd/policy.json`，`ci-gate`/`pr-title` 均成功。首轮 canonical accept 的 `FILESYSTEM_ERROR` 只保留为失败事实，trusted main 随后以 canonical retry 成功，未补造 receipt。PR #180 fixed head `f972a3c78b5aa119784b871c7bfc4b43d72bdac5` 经 3600 秒 closeout monitor 后以 `a133a79c819ff875cfffca40967700679b4fc383` 合并。C 的 adapter 读取历史任务但不把 Trellis 路径传给 GKD migrate-v1；原始 AIO 未跟踪计划目录、生产和 GitHub settings 均未修改。下一步 D 必须重新从已发布 v0.1.5 asset 建立隔离 runtime。
...
- [2026-08-30] `GKD-O4-WATCHER-HISTORICAL-LANE-R4` 阻塞尝试已清理。
  - Why: candidate tree 在 block 后保持干净，未创建 PR 或远端 task branch。
  - Impact: candidate worktree 与本地 branch 已删除，runtime 和 planning package 已移入可恢复 Trash；R5 从记录该事实后的 trusted main 建立。
...
- [2026-08-31] O8 将扩展兼容矩阵迁入显式 release-upgrade lane，并保持 release/finalization engine 分离。
  - Why: 每个 core PR 重跑稳定版本、组合与扩展恢复矩阵会增加验证成本，但公开旧格式仍须在默认路径有可定位的 read 与 reject/restore 合同。
  - Impact: 版本化 catalog 将十种公开格式的 core 和 matrix test ID 严格绑定；release-upgrade/matrix 独立绑定 scope、head、base、environment 和 result digest。ADR-001 明确本任务不提取或合并 `gkd-finalize` 与 `gkd-release`，未来迁移须以独立授权和双解释器兼容合同开始。
...
- [2026-09-02] 测试只覆盖真实工作流风险，并要求设计/实现完成后的消融实验。
  - Why: 对零碎实现细节新增单元测试会制造维护负担，却不能提升用户工作流的可信度；无用抽象和设计会重新引入 manual-first 要消除的复杂度。
  - Impact: 仅在改动影响真实用户工作流或高风险核心行为时保留必要的端到端验证；每次设计或实现结束后，主动删除不必要的抽象和设计。
