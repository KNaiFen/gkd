...
- 项目 `.codex/agents/` 已定义 `gkd_execute`、`gkd_ci_monitor`、`gkd_accept` 三个角色；执行/验收固定为 `gpt-5.6-sol` / `xhigh`，CI 监控固定为 `gpt-5.6-terra` / `high`，并固定 sandbox、提示词和禁止嵌套边界。main 启动执行时必须使用命名 `agent_type=gkd_execute`，不能退回泛化默认子代理。
...
- `plan-only` 只允许调查、问答和写 PLAN；只有用户明确批准“按此 PLAN 开始执行”后，main 才能创建 worktree/分支、启动角色或写代码。需要等待 CI 时必须由 `gkd_ci_monitor` 单目标显式调用 `scripts/gkd-github-watch --interval 30 --timeout 3600`；改变 interval 或 timeout 任一参数须有 PLAN 授权，main 一次性等待终态。
- delegated 执行完成后才可由 `gkd_accept` 独立验收；成功收尾固定为 main 写 review、创建并检查脱敏归档、按授权创建包含活动记录删除的 cleanup commit、审查/合并、清理已确认合并的本轮任务分支、恢复干净 `main`、输出详细报告；失败、未授权或远端状态不明时保留现场。临时 `gkd-legacy-cleanup` 只按明确老项目和逐项授权盘点/清理旧活动机制，不触碰普通业务、历史归档或生产用户目录。
