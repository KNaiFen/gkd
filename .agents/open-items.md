...
- [x] `GKD-M3-A` 候选已在 PR #8 实现并修复 shallow checkout 的 full-base 验证、通用 `.gkd` policy schema、repo/policy/origin 一致性、GitHub fixed-head terminal monitor、标准 Actions/verifier；335 项本地验证与 29 项双 evidence 通过，候选 bundle digest 为 `0484095704599750df655bc6c92cf0b5829bc2c1ebb877aa3f3cd132cc29998f`，等待最终 fixed-head CI 与 trusted-main 独立验收。
