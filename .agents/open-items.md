...
- 当前 CI 约束：需要等待时由 `gkd_ci_monitor` 对单一目标显式调用 `scripts/gkd-github-watch --interval 30 --timeout 3600`；改变任一参数须有 PLAN 授权，main 一次性等待终态，缺失/漂移/认证错误即阻塞。
- 当前收尾约束：已批准 delegated 执行完成后才由 `gkd_accept` 独立验收；成功必须先 review 和脱敏归档，再按授权创建包含活动记录删除的 cleanup commit，经过审查/合并后才清理已确认合并的本轮任务分支，恢复干净 `main`，最后输出详细报告；异常或远端状态不明时保留现场。`gkd-legacy-cleanup` 仅按明确老项目和逐项授权使用。
