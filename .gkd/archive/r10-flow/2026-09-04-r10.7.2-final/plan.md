...
本节是针对 session `01a0689d-d152-7f60-a4a6-a23fddf1fbc0` 暴露问题的修订草案背景。原“待用户继续讨论和确认、只授权规划”的状态已被 r10.6 取代；当前授权和施工状态以文件顶部状态区及最新 `plan-changes.md` 为准。
...
成功标准：对临时 fixture 完成一次只读盘点演练，所有活动旧 GKD Skill/入口/引用均有明确分类和授权处理结果；Skill 明确目标项目不再有可执行的旧 GKD 路由或状态机制，但本轮不对真实老项目执行删除；报告列出保留的历史记录和未决证据缺口。
...
- 远端任务分支删除属于 GitHub 写操作；本轮已获得用户对施工完成后清理本地和远端任务分支的明确授权，但仍只处理本轮任务分支且须在验收、归档和合并条件满足后执行。项目活动 PLAN 文件的删除仅限已归档且确认只属于本轮的文件。
...
独立验收发现 r10.6 的规则仍有可执行性缺口，本轮返工只补齐以下内容，不扩展为旧自动化机制：

- 明确 CI Skill/role 显式传入 `--interval 30 --timeout 3600` 才是本流程规范；脚本自身默认值不作为 GKD 约束来源，`interval` 与 `timeout` 均须由 PLAN 授权后才能改变，并在验证中检查调用参数。
- 在 progress/review 中记录可审计的计划闸门、材料性变更重新确认、delegated/direct-main 收尾和 review revision 手工演练；不引入运行时状态机。
- 把归档和活动记录清理改成可落地的提交顺序：先生成并检查归档，再在已授权提交的任务分支创建 cleanup commit（删除仅属于本轮的活动记录），再由 main 审查/合并；未获提交授权或条件不满足时保留现场，不宣称 clean main。
- 所有面向用户和主流程的角色文案明确命名 `agent_type=gkd_execute`；`gkd_accept` 仅能在已批准的 delegated 任务执行完成后启动。
- 同步持久记录和 ADR 中与本轮规则冲突的“可归档/泛化角色”表述；仅做文案一致性修正，不新增运行时机制。
- 将清理 Skill 的活动引用定义为“仍指向已确认旧 GKD 机制的可解析引用”，与失效链接一起按授权处理；历史事实记录和归档仍保留。
