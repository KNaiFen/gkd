...
| 2026-09-04-r10.7 | `gkd_accept` 独立验收发现演练无证据、CI 脚本默认值与规范不一致、归档后删除 tracked 活动记录缺少 cleanup commit 顺序、角色文案存在泛化绕行。 | 明确 CI 显式参数规范和 interval/timeout 授权；补充可审计演练记录；定义“归档检查 -> 已授权 cleanup commit -> main 审查/合并 -> 清理分支”的顺序；收紧命名角色和 `gkd_accept` 前置条件。 | 不新增远端写操作授权；cleanup commit/合并仅在已有交付授权时执行，否则保留现场并报告阻塞。 | execution revision 更新为 r10.7，要求记录演练和新的 cleanup 顺序。 |
| 2026-09-04-r10.7.1 | 独立验收进一步发现持久记录/ADR 泛化措辞、真实老项目清理成功标准超出本轮范围，以及清理 Skill 未明确可解析的旧机制活动引用。 | 把清理成功标准改为临时 fixture 盘点演练；同步持久记录和 ADR 的文案；将已确认旧机制的活动引用与失效链接一并纳入授权清理。 | 仅做同一流程目标下的文档一致性修正，不新增真实老项目删除或外部写操作授权。 | execution revision 更新为 r10.7.1。 |
| 2026-09-04-r10.7.2 | 第二轮验收发现活动 PLAN/review/template 未同步最新 revision，手工流程存在旧参数表述，cleanup 顺序在主计划中仍过时，且 `gkd_accept` TOML 前置条件缺失。 | 将当前指针更新为 r10.7.2；同步 review 模板、手工流程、`gkd_accept` 配置和证据表述；把 cleanup commit -> main 审查/合并 -> 分支清理写入主计划。 | 仅补齐同一已批准流程的文档/角色约束；未新增真实老项目删除、CI 写操作或发布授权。 | execution revision 更新为 r10.7.2。 |
| 2026-09-04-r10.7.3 | 最终验收发现活动 PLAN/review 当前指针仍滞后、review 模板虽已修改但未列入 execution 清单，以及 fixture 证据含本机临时路径和未展示本地分支删除命令。 | 将活动 PLAN/review 切换到 r10.7.2 当前结论；把 review 模板纳入 execution 允许范围；将 fixture 路径泛化为 `<temp-fixture>`，补充 `git branch -d` 命令/断言或标记未验证。 | 仅修正记录和证据可审计性，不新增施工、真实删除、远端写操作或发布授权。 | main 更新活动记录；无需重新启动执行代理。 |
