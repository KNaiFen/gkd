...
class PackagingContracts(unittest.TestCase):
    ...
    def test_two_temporary_installs_have_same_version_digest_and_inventory(self) -> None:
        ...
        first.mkdir()
        second.mkdir()
        first_install = gkd_bundle.install(self.source, temporary_root, first, ("legacy-automatic",))
        second_install = gkd_bundle.install(self.source, temporary_root, second, ("legacy-automatic",))
    ...
    def test_installed_gkd_task_is_executable_and_imports_installed_library(self) -> None:
        ...
        target.mkdir()
        runtime.mkdir()
        gkd_bundle.install(self.source, temporary_root, target, ("legacy-automatic",))
    ...
    def test_task_schemas_are_strict_versioned_and_installed(self) -> None:
        ...
        expected = {"task-state.schema.json", "offer.schema.json", "authorization.schema.json", "result-manifest.schema.json", "runtime.schema.json"}
    ...
    def test_manifest_declares_separate_task_cli_library_and_schemas(self) -> None:
        ...
        self.assertEqual(
            len([path for path in (self.source / "payload").rglob("*") if path.is_file()]),
