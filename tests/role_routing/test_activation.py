...
from gkd_role.activation import TrustedActivationEvidenceProvider, TrustedMainActivationAuthority
...
class ActivationContracts(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.authority = TrustedMainActivationAuthority(self.runtime, self.catalog)

    def tearDown(self) -> None:
        self.repo.close()

    def record(self, expected=None, observation=None):
        return self.authority.record(expected or self.expected, observation or self.observation, "activation-nonce")

    def provider(self, activation_id, catalog=None):
        return TrustedActivationEvidenceProvider(self.runtime, activation_id, catalog or self.catalog)
    ...
    def test_main_owned_activation_claims_exact_offer_and_delivers(self) -> None:
        activation = self.record()
        provider = self.provider(activation["activationId"])
        service = TaskService(self.repo.candidate, self.repo.task_path, self.runtime, FixedClock(FIXED_TIME), evidence_provider=provider)
        claimed = service.claim(*self.repo.cas(), self.handoff["envelopeId"])
    ...
    def test_candidate_public_cli_and_library_paths_remain_fail_closed(self) -> None:
