...
class PackagingContracts(unittest.TestCase):
    def test_manifest_declares_every_role_routing_payload_and_mode(self) -> None:
        ...
        self.assertTrue({"role-routing-cli", "role-routing-library", "role-routing-source", "role-routing-schemas", "legacy-workflow-skills"}.issubset(names))
        self.assertEqual(
    ...
    def test_role_and_task_schemas_are_versioned_and_strict(self) -> None:
        ...
        self.assertEqual({"activation.schema.json", "context.schema.json", "project.schema.json", "route-decision.schema.json", "route.schema.json", "spawn-result.schema.json", "wait.schema.json"}, role_schemas)
    ...
    def test_install_exercises_role_cli_and_has_exact_inventory_modes(self) -> None:
        with tempfile.TemporaryDirectory(prefix="gkd-role-install-") as root_name:
            ...
            target.mkdir()
            installed = gkd_bundle.install(SOURCE_ROOT, temporary, target, ("legacy-automatic",))
            lock = json.loads((SOURCE_ROOT / "manifest.lock.json").read_text(encoding="utf-8"))
            ...
            selected_files = [item for item in lock["installFiles"] if item.get("pack") in (None, "legacy-automatic")]
            self.assertEqual(len(selected_files) + 4, installed["files"])
            self.assertEqual(["legacy-automatic"], installed["installedPacks"])
    ...
    def test_installed_optional_pack_enables_explicit_role_context(self) -> None:
        with tempfile.TemporaryDirectory(prefix="gkd-role-pack-") as root_name:
            ...
            target = temporary / "target"
            target.mkdir()
            gkd_bundle.install(SOURCE_ROOT, temporary, target, ("legacy-automatic",))
            executable = target / "gkd" / "bin" / "gkd-role"
    ...
    def test_role_cli_no_longer_exposes_migration_commands(self) -> None:
        with tempfile.TemporaryDirectory(prefix="gkd-role-no-migration-") as root_name:
            temporary = Path(root_name)
            target = temporary / "target"
            target.mkdir()
            gkd_bundle.install(SOURCE_ROOT, temporary, target, ("legacy-automatic",))
            executable = target / "gkd" / "bin" / "gkd-role"
            for command in ("migration-plan", "production-migration-plan"):
