...
每个 delegated 任务使用五份 Markdown 记录：

- `plan.md`：main 的方案、技术栈、实现思路、范围和授权；
- `execution.md`：目标 worktree 内当前轮次的执行交接，执行 session 只读取它；
- `progress.md`：已完成事项、判断、阻塞、风险和下一步，由执行代理维护；
- `plan-changes.md`：main 对方案调整的追加式思路记录；
- `review.md`：main 的独立验收结论和返工意见。

标准顺序是：main 选择 direct-main，或创建计划和 worktree；委派任务默认由用户手动启动执行 session，只有用户明确选择才由 main 自动启动；main 生成 worktree 内 `execution.md`，执行 session 按它工作并更新进度，完成后通知 main；main 检查 diff、五份记录和必要的局部验证，随后通过，或记录 review、修改 plan、追加 plan-changes、更新 execution 后继续执行。任务完成后可把记录摘要归档到目标项目的 `.gkd/archive/`。详见 [Manual-first 工作流](docs/manual-workflow.md) 及 [VISION](VISION.md)。
