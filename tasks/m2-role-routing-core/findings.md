...
- 结论：F-005 已整改；F-004 v4 唯一 live probe 未产生 custom-role activation，继续阻塞
...
- 状态：v4 正常环境静态预检通过；唯一 live probe 已执行但未启动 `gkd_executor`
- 严重程度：阻塞
- 返工责任：执行 session
- 对应要求：requirements AC12；plan Acceptance 91-92；delivery Handshake Boundary
- 证据：`evidence/m2-role-routing-core/role-handshake.json` 以 schema v2 绑定 deterministic preflight 与 host facts。v4 严格解析生成的 project config 与 `gkd_executor.toml`，并以非 strict app-server 到达 no-transport；role/config/bundle/Skill/project digest、冻结参数、临时 repo 和生产配置非漂移均固定。唯一真实调用 `codexExitCode=0`、`modelInvocations=1`、`liveAttemptsConsumed=1`；host 事件证明 parent turn 进入并 terminal，但唯一 collaboration tool 为无 receiver/agent state 的 `wait`，没有 spawn、activation、child identity 或 child terminal。分类为 `CUSTOM_ROLE_ACTIVATION_MISSING`。v3 strict 失败与此前隔离模式 HTTP 400 分别保留为历史兼容性/负向证据。
...
本轮 v4 live 授权已消耗，禁止再次调用。宿主未证明唯一 `gkd_executor` activation 或 child terminal，因此 F-004 和总体继续 `blocked`；PR 仍不得由 execution session 自行验收或合并，也不得进入 M2-B 或 automatic route。
