...
- 本任务从 trusted main `ba32d14729eb38058f4d59e9c83b3e22ff0c8993` 建立，execution bundle 固定为 `04efd9ce5f1e0f678f9853eef5d9fb20606fff6e667aba69d9b204bddeb9b5d6`。
- O4、R1-R5 与 compatibility attempt 0/R1/R2 lifecycle 都只读归档；R5 diff 可只读参考，禁止复用其 task、offer、claim、runtime、branch、PR、delivery 或 result artifacts。compatibility R3 merge `aeeeb2b57fc98289e341f4b04790b7cf34d78ee3` 是本任务的 accepted consumer 基线。
- 只允许一个精确 `gkd_executor` 交付、一个独立 `gkd_acceptor` 验收和 trusted main 合并/清理。executor 必须执行 bridge trusted-main-only execution context 的精确 argv，不依赖 PATH 或 cwd 推断；host acknowledgement 在 spawn 前准备，spawn 返回后立即 claim。
...
- result manifest 继续记录已知 lane/profile 与完整 scope 集合；已合并的 consumer、delivery artifact validation、acceptance 和 rework 从 fixed tree 严格验证该声明，而不以旧全局 scope 集合推断 default 必需项。
...
- 不删除 fixtures/evidence、不拆 O5-O8，也不再改变 gate-repair 或 Python 3.9 runtime 契约。
