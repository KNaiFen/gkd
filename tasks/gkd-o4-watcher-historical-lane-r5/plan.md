...
- 基线为 `edf0f5c316d828594df58197c043cbc7ee74defb`，execution bundle 为 `b0174e6c154c22dd73975857e084e26d095f7fb73e5b80588bb7a8a8f697a618`。
- R5 是新的 task/offer/claim/runtime/branch/worktree/PR；历史 lifecycle 不重用。
- executor 使用固定绝对 CLI `/usr/bin/python3 -B /private/var/folders/dv/7psz5djd3537ghdrhkpzy7dw0000gn/T/gkd-o4-r2-execution-bundle/source/gkd/bin/gkd-task`；不依赖 PATH。
...
- 修改 default scope 列表、historical runner、manifest producer/consumer、delivery/acceptance/rework validation、README、tests、schema、manifest/lock。

## Non-Goals

- 不重写或删除 watcher/probe，不删历史证据，不启用 automatic route，不进入 O5-O8，不更改 R6/Python 3.9 合同。
...
- gkd-main 完成 planning、authorization、offer、claim 和 trusted bridge；精确角色为 `gkd_executor`，无 worker/fallback/nested/retry。
