...
- 基线为 trusted main `528959a97bf9c4f5abe0c7dbe3b841beff15e074`，execution bundle 为 accepted R6 bundle `b0174e6c154c22dd73975857e084e26d095f7fb73e5b80588bb7a8a8f697a618`。
- O4 和 O4-R1 lifecycle 只读归档；只使用新的 task、offer、claim、runtime、branch 与 worktree。
...
- 不改动 R6 的逻辑顺序、planning refresh、delivery result-manifest 或 Python 3.9 compatibility 契约。
...
- 系统 Python 3.9.6 与开发 Python 3.14.6 都通过完整 verifier、bundle 与 delivery 路径验证。

## Compatibility

- `gkd_watchdog` Python API、`gkd-watchdog-mcp` CLI、M-1B evidence schema 和 M-1C live probe 参数保持兼容。
- 默认验证器的退出码、base SHA ancestry 校验、canonical result schema 和 protected/output 边界保持 O3/R6 语义。
...
- final implementation commit 前完成代码、schema、lock、双解释器验证和 required artifacts；delivery document 必须是其直接子提交，delivery 后不再提交。
