...
- 本任务从 R6 收尾后的 trusted main `528959a97bf9c4f5abe0c7dbe3b841beff15e074` 建立，execution bundle 固定为 `b0174e6c154c22dd73975857e084e26d095f7fb73e5b80588bb7a8a8f697a618`。
- O4 与 O4-R1 的 rejected lifecycle 都是只读历史；必须建立新的 task、offer、claim、runtime、branch 和 candidate worktree，不得 rework、修补或复用旧对象。
...
- 不再次修改逻辑时钟、planning refresh、delivery sidecar、task state schema 或 Python 3.9 runtime 兼容实现。
...
6. 系统 Python 3.9.6 与开发 Python 3.14.6 均可完成本任务的完整 verifier、bundle 与 delivery 路径验证。
7. 变更不引入绝对本机路径、用户名、凭据、新依赖或生产/AIO/GitHub settings 副作用。
