# GKD Gate Repair R1 Plan

## Goal

把生命周期排序、规划文档刷新和交付结果绑定收敛为三个由 canonical 服务生成和验证的门禁，并让 schema/packaging 与最终 delivery head 一次性闭合，使跨进程 GKD 任务可以恢复、返工并在固定事实上交付。

## User Decisions

- 以 trusted main `9a04e3f` 为完整基线，独立任务完成后才重启 O4；attempt 0 的 rejected 状态只读保留。
...
- 最终 bundle digest 只生成一次并在 delivery 前写入 result manifest、delivery 文档和 task state；delivery 后不得刷新或修补 bundle。
...
- 更新 packaging expected-set 和 bundle lock 的同 head 校验，确保 trusted main 的 validator 能读新状态。
...
- O4 必须从本任务 accepted merge 的完整 SHA 重新 bootstrap；旧 O4/O4-R1 和 attempt 0 的 blocked/rejected 记录只读保留。
...
- 明确 manifest 的 canonical 文件格式、改变路径集合、digest 计算顺序和自动/手动路线差异；为逻辑顺序、document refresh、packaging expected-set 和 delivery-after-commit drift 添加 mutation kill tests，防止只改生产代码而合同未覆盖。
