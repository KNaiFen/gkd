# GKD Gate Repair R1 Requirements

## Goal

修复 GKD 任务协调层在跨进程执行时暴露的三个门禁缺陷：生命周期事件的 wall-clock 逆序、规划文档 digest 在受控变更后的刷新死锁、delivery 与 candidate result manifest 的绑定不足。R1 还必须修复 attempt 0 暴露的 schema/packaging 不一致，并保证所有实现、测试、bundle manifest 和交付绑定在 delivery 前完成。修复必须保持单一事实源、单 writer、固定 head 和 fail-closed 语义，并让后续 O4 可以从新任务重新开始。

## User Decisions

- 本任务从 trusted main 当前基线 `9a04e3f` 建立，作为 O4 重启前置任务；不得手改或复活已阻塞的 O4/O4-R1/attempt 0 状态。
...
- packaging expected-set、manifest/lock、schema digest 和 candidate output digest 必须在同一 implementation head 内同步；delivery coordination commit 之后禁止任何代码、schema、测试或 manifest 提交。
...
5. 相关 task-core、runtime-bridge、rework、结果消费者、packaging expected-set 和 schema 合同通过；候选 bundle、manifest/lock、delivery 记录和测试摘要的 digest/固定 head 一致，且最终 fixed head 与 delivery coordination head 相同。
6. delivery 之后 candidate worktree 只能保持 clean，任何后续修改都必须被 canonical rework 退役并新建 offer/claim；acceptor 能从 trusted main 读取并验证最终 task state。
7. 变更不引入绝对路径、凭据、新外部依赖或生产/AIO/GitHub settings 副作用；失败、取消和重试路径不留下不可恢复的 coordination 半状态。
