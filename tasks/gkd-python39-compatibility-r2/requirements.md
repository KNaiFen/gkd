...
- 本任务为 R1 delivery block 后的独立 R2，从 trusted main `f4bdab2ecad412dc42b6b85e8fc18c5162369d4c` 建立。旧 attempts 与 GKD-GATE-REPAIR attempt 0 至 R5、O4 都是只读历史，禁止复用其 offer、claim、activation、delivery 或 PR；R1 implementation `c7d7e47c702c23119204fac9a1121b253a86c27e` 只可作为重建源码的只读参考。
...
- 修复并覆盖 exact trusted bridge claim 后，candidate 在系统 Python 3.9 对其真实 claim/activation receipt 的 canonical `deliver` 路径；receipt 只能由 bridge/service 生成、读取和验证，任何缺失、篡改、错 task、错 head 或错 runtime 仍 fail-closed。
...
- 不手写、复制、放宽或恢复 runtime receipt，不把 R1 的 lifecycle/runtime 迁入本任务。
...
8. 从 fresh trusted bridge offer/claim 出发，系统 Python 3.9 的 candidate CLI 成功完成 canonical `deliver`，生成唯一 final lifecycle commit；receipt 缺失、篡改、runtime/key/head/task 漂移保持 `CLAIM_RECEIPT_UNAVAILABLE` 或既有明确拒绝，且不写半状态。
