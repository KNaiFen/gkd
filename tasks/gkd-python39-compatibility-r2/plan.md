...
- 用户选择完整 Python 3.9 兼容移植；本任务是 R1 delivery block 后的独立 R2，基线为 trusted main `f4bdab2ecad412dc42b6b85e8fc18c5162369d4c`。
...
- fresh trusted bridge claim 后，candidate 使用系统 Python 3.9 执行 `deliver` 必须验证同一 canonical claim/activation receipt；真实 receipt 存在时不得误报 unavailable，任何漂移仍 fail-closed。

## Scope

- 更新 payload、watcher/probe、测试运行器、manifest/lock、许可和最低版本文档。
- 增加 Python 3.9 subprocess/full-verifier、TOML parity/negative、CLI 分类和 fresh bridge claim-to-deliver receipt 合同。
...
- 不手工写入、复制或恢复任何 activation/claim receipt，不迁移 R1 lifecycle/runtime。

## Acceptance Criteria

- Python 3.9 与开发解释器均通过完整 verifier、bundle 和核心 CLI；TOML、严格配对与 fresh bridge claim-to-deliver receipt 保留 fail-closed 行为。
...
- 先重现 R1 receipt 误拒绝，再完成枚举范围的最小兼容改动；生成 package/lock 后双解释器复验，并以 fresh bridge claim 运行 system Python 3.9 canonical deliver 正反合同。
