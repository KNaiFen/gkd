...
- Epoch 2 必须修复 rejected finding P2：project stage 以 selected packs 生成实际 `gkd_executor.toml`，并把 pack-aware role/config 与 inventory digest 绑定到同一受管状态。测试必须解析生成 TOML，确认未选 pack 不出现、每个选中 pack 的 Skill 启用条目精确出现，不能只确认文件已复制。
