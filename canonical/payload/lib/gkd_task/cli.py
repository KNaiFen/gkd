...
def _parser() -> MachineParser:
    ...
    deliver.add_argument("--result-manifest-path")
    deliver.add_argument("--result-manifest-digest")
...
def _service(args: Any) -> TaskService:
    ...
    return TaskService(
        ...
        allow_document_drift=getattr(args, "command", None) == "planning-refresh",
    )
