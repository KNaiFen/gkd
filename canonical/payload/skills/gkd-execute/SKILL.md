...
5. Generate the candidate output bundle and report its digest separately; never replace the execution bundle digest of the in-flight claim. Prepare the canonical `tasks/<task>/delivery.md`, commit that document alone, capture that commit's lowercase full SHA and the current revision, then invoke `gkd-task deliver` with that SHA as its expected head, the document path/digest, and the candidate-output bundle digest. The CLI alone creates the final state commit.
