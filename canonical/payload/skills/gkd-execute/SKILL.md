...
5. Generate the candidate output bundle and report its digest separately; never replace the execution bundle digest of the in-flight claim. Prepare the canonical `tasks/<task>/delivery.md`, commit that document alone, then invoke `gkd-task deliver --delivery-document-path tasks/<task>/delivery.md --delivery-document-digest <sha256> --candidate-output-bundle-digest <sha256>` against that document commit.
