...
5. Generate the candidate output bundle and report its digest separately; never replace the execution bundle digest of the in-flight claim. Commit all implementation changes first. Prepare the canonical `tasks/<task>/delivery.md`, commit that document alone, then invoke `gkd-task deliver --expected-head <delivery-document-commit-full-sha> --delivery-document-path tasks/<task>/delivery.md --delivery-document-digest <sha256> --candidate-output-bundle-digest <sha256>`. The full delivery-document commit SHA, never the implementation commit SHA, is the delivery CAS value.
