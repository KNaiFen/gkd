...
5. Generate the candidate output bundle and report its digest separately; never replace the execution bundle digest of the in-flight claim. For automatic delivery, commit the canonical `tasks/<task>/result-manifest.json` sidecar from the implementation head before preparing `tasks/<task>/delivery.md`. Commit the delivery document alone, capture that commit's lowercase full SHA and the current revision, then invoke `gkd-task deliver` with that SHA as its expected head, the document path/digest, and the candidate-output bundle digest. The CLI derives the sidecar binding and alone creates the final state commit.
