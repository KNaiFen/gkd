...
5. Obtain the six-gate decision from `gkd-role route` with the verified project-policy record, then invoke `TrustedMainRuntimeBridge.prepare_handoff` with the trusted project and production roots. The returned handoff seals execution context, spawn request, offer/envelope bindings, and claim CAS before the host call. Spawn exactly one direct `gkd_executor` from its request with `fork_turns=none`; never substitute `worker`, another role, nested Codex, downgrade, or fallback.
6. Normalize only one successful host acknowledgement by passing it to the sealed handoff: one direct `gkd_executor` call and `fork_turns=none`, with the host's canonical `/root/<requested-task-name>` preserved as the task identity. Do not strip, reconstruct, or substitute it, or reconstruct agent IDs, thread digests, effective model/effort/sandbox/runtime, timestamps or fallback evidence the host did not return. The handoff is single-consume even on rejection and performs the bound claim without a second activation/context message. The bridge binds configured role values from the verified bundle and derives a non-secret executor-attempt handle. Use `TrustedMainRuntimeBridge.recover` only after an interrupted claim transaction or receipt completion. Public `gkd-role automatic-*`, candidate `gkd-task claim`, and default task-library claim paths are unavailable trust boundaries and must remain byte-unchanged on rejection.
