...
5. Invoke only the trusted `gkd-task accept` path after all gates pass. For GitHub, pass the installed `gkd-github-accept` executable as `--adapter-command`; it uses canonical JSON, GitHub REST reads, and an exact-head squash merge. If the canonical outcome is rejected with stable non-empty findings, use only the trusted `gkd-task rework` path at that same fixed head; it records the rejected attempt and returns authority to planning without editing implementation files.
