...
GKD 是用户使用 Codex 修改项目的统一入口。它先负责需求对齐和实现就绪 PLAN，再按用户确认的路线调用执行、CI 监控和验收角色，最后按授权完成交付。本文规定其中默认的 manual-first 协作方式；它只约定协作材料和工作顺序，不是机器状态机，也不要求填写 JSON、digest、CAS 或专用命令参数。执行 session 默认由用户手动启动；用户明确选择后，main 可以用当前 Codex 已配置的角色启动一个普通子代理。
